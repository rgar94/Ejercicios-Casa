//Semestre 2019 - 2
 ...
 Visual Studio 2017
 Práctica 8
 Manual: Con el uso de las funciones para cargar y aplicar las texturas a una mesa y un tablero de ajedrez, 
 cuidando en este caso que la parte del tablero de ajedrez se repita la imagen 4 veces verticalmente y 4 veces horizontalmente
 para que asi quede de forma correcta.
 */
#include "texture.h"
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	t_cube.LoadTGA("window.tga");
	t_cube.BuildGLTexture();
	t_cube.ReleaseImage();

	t_wood1.LoadTGA("madera1.tga");
	t_wood1.BuildGLTexture();
	t_wood1.ReleaseImage();

	t_wood2.LoadTGA("madera2.tga");
	t_wood2.BuildGLTexture();
	t_wood2.ReleaseImage();

	t_wood3.LoadTGA("madera3.tga");
	t_wood3.BuildGLTexture();
	t_wood3.ReleaseImage();
...
void prisma(float x, float y, float z,GLuint textura1,GLuint textura2)
{
	GLfloat vertice[8][3] = {
...
void prisma2(float x, float y, float z)
{
	...
	glTexCoord2f(1.0f, 1.0); glVertex3fv(vertice[6]);
	glTexCoord2f(1.0f, 0.0f); glVertex3fv(vertice[2]);
	glEnd();
	glBegin(GL_POLYGON);  //Bottom
	glNormal3f(0.0f, -1.0f, 0.0f);
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(pos_camX, pos_camY, pos_camZ);
		...
		glPushMatrix(); 
		glTranslatef(0,-2,0);
		prisma(0.5,4,0.5,t_wood2.GLindex, t_wood2.GLindex);
		glPopMatrix();
		glPushMatrix();
		glTranslatef(0, 0.5, 0);
		prisma(5, 1, 5, t_wood2.GLindex, t_wood1.GLindex);
		glPopMatrix();
		glTranslatef(0, 1.25, 0);
		prisma(3, 0.5, 3, t_wood3.GLindex, t_Ajedrez2.GLindex);
	
		//Silla1
		glPushMatrix();
		glTranslatef(3, -4, 0);
		prisma(0.5, 2, 0.5, t_wood3.GLindex, t_wood3.GLindex);
		glTranslatef(1, 0, 0);
		prisma(0.5, 2, 0.5, t_wood3.GLindex, t_wood3.GLindex);
		glTranslatef(0, 0, 1);
		prisma(0.5, 2, 0.5, t_wood3.GLindex, t_wood3.GLindex);
		...
		prisma(2, 0.5, 2, t_wood3.GLindex, t_wood3.GLindex);
		glTranslatef(0.875, 1.25, 0.75);
		prisma(0.25, 2, 0.25, t_wood3.GLindex, t_wood3.GLindex);
		glTranslatef(0, 0, -1.5);
		prisma(0.25, 2, 0.25, t_wood3.GLindex, t_wood3.GLindex);
		glTranslatef(0, 0, 0.75);
		glBindTexture(GL_TEXTURE_2D, t_cube.GLindex);
